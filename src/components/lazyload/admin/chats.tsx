...
// Fake Data
import { conversations } from "@/mock-data/chats"

type ChatUser = (typeof conversations)[number]
type Convo = ChatUser['messages'][number]

export default function ChatsLazyAdmin() {
  const [search, setSearch] = useState('')
  const [selectedUser, setSelectedUser] = useState<any>(conversations?.[0])
          ...
          <div className='flex w-full flex-col gap-2 sm:w-56 lg:w-72 2xl:w-80'>
            ...
            <ScrollArea className='-mx-3 h-full p-3'>
              {filteredChatList.map((chatUsr) => {
                ...
                const lastConvo: any = messages[0]
                const lastMsg =
                  ...
                  <div className='chat-flex flex h-40 w-full flex-grow flex-col-reverse justify-start gap-4 overflow-y-auto py-2 pb-4 pr-4'>
                    {currentMessage &&
                      Object.keys(currentMessage).map((key) => (
                        <Fragment key={key}>
                          {currentMessage[key].map((msg: any, index: any) => (
                            <div
